...
def main() -> None:
    ...
    for c in c_values:
        ...
        for thr in thresholds:
            ...
            eligible = val_metrics["trades_taken"] >= min_validation_trades
    ...
    if best_clf is None:
        raise RuntimeError(
            "No valid configuration selected. Try lowering min_validation_trades."
        )

    print("\n=== Best validation-selected setup ===")
    print(best_params)
    print(f"Validation trades: {best_val_metrics['trades_taken']}")
    print(f"Validation precision: {100 * best_val_metrics['precision']:.2f}%")
    print(f"Validation recall: {100 * best_val_metrics['recall']:.2f}%")
    print(f"Validation accuracy: {100 * best_val_metrics['accuracy']:.2f}%")
    print(f"Validation profit: {100 * best_val_profit:.2f}%")

    best_test_scores = best_clf.decision_function(x_test)
    best_y_pred = (best_test_scores > best_params["threshold"]).astype(int)

    print("\n=== Final test result for validation-selected setup ===")
    evaluate_and_print_trade_zhongjie(
        "Linear_SVM_c1_confirm",
        y_test,
        best_y_pred,
        take_profit=cfg.take_profit,
        stop_loss=cfg.stop_loss,
